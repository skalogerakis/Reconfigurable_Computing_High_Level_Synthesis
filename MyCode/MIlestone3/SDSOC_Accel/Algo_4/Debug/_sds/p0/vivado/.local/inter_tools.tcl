...
set initName {}
set newName {}
# Mode is always impl now with single project flow. We don't use this below,
# but the Vivado GUI still calls an API that depends on it.
set mode impl
...
proc get_run { mode } {
  if {[get_param project.sdxInteractiveUseCurrentRun]} {
    if {$mode eq "synth"} {
      set catch_result [catch {current_run -synthesis} run]
      if {$catch_result} {
        set run [get_runs $::sdx::primary_synth_name]
      }
    } else {
    ...
    if {$mode eq "synth"} {
      set runName $::sdx::primary_synth_name
    } else {
      set runName impl_1
    }
    set run [get_runs $runName]
  }
  return $run
}
...
proc close_project_actions {} {
  ...
  # For synth mode, the project should start with a single synth run,
  # ::sdx::primary_synth_name, # and a corresponding implementation run,
  # impl_1. After that, the user can create new runs, eventually deciding on
