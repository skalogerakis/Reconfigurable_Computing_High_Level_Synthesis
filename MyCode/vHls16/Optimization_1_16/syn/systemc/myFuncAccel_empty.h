...
#ifndef _myFuncAccel_empty_H_
#define _myFuncAccel_empty_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "myFuncAccel_empty_memcore.h"

SC_MODULE(myFuncAccel_empty) {
...
protected:
    ...
    // buffer instances
    myFuncAccel_empty_memcore* buffer[BufferCount];
...
public:
    ~myFuncAccel_empty();
    SC_CTOR(myFuncAccel_empty) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "myFuncAccel_empty_memcore_U_";
            name += szTmp;

            buffer[i] = new myFuncAccel_empty_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "myFuncAccel_empty.trace");
            // ports
